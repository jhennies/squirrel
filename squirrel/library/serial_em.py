...
class Navigator:
    ...
    def __init__(
        ...
        self.map_items = dict()
        self.map_item_keys = dict()
        for map_type in self.map_types:
            keys, items = self.get_map_items(map_type)
            self.map_items[map_type] = items
            self.map_item_keys[map_type] = keys
    ...
    def get_map_filepaths(self, map_type):
        return self._function_on_property(map_type, 'map_items', self.get_map_filepath)

    def get_mdoc_filepath(self, filepath):
        return get_mdoc_filepath(filepath)

    def get_mdoc_filepaths(self, map_type):
        return self._function_on_property(map_type, 'map_filepaths', self.get_mdoc_filepath)
    ...
    def get_map_full_affines(self, map_type, stage_coordinate_system=False):
        ...
        kwargs = dict(
            apply_affine=(
                ...
                self._get_map_full_affine(self.map_items['grid'][0], self.map_binnings['grid'], invert=False, full_square=True, flatten=False)
            ),
            binning=self.map_binnings[map_type],
            invert=True, full_square=True, flatten=True
        )
        return self._function_on_property(map_type, 'map_items', self._get_map_full_affine, **kwargs)
...
class SingleParticleNavigator(Navigator):
    ...
    def get_view_map_filepath(self, item):
        for zero_padding in range(4):
            fp = get_view_map_filepath(item, self.filepath, pad_zeros=zero_padding)
            if os.path.exists(fp):
                return fp

    def get_record_map_filepath(self, item):
        for zero_padding in range(4):
            fp = get_record_map_filepath(item, self.filepath, binning=self.map_binnings['record'], pad_zeros=zero_padding)
            if os.path.exists(fp):
                return fp

    def get_map_filepaths(self, map_type):
        if map_type == 'grid':
            return self._function_on_property(map_type, 'map_items', self.get_grid_map_filepath)
        if map_type == 'search':
            return self._function_on_property(map_type, 'map_items', self.get_search_map_filepath)
        if map_type == 'view':
            return self._function_on_property(map_type, 'map_items', self.get_view_map_filepath)
        if map_type == 'record':
            return self._function_on_property(map_type, 'map_items', self.get_record_map_filepath)
